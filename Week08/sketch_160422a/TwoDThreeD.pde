...
List<PVector> sortedIntersections =  sortCorners(intersections); 
...
class TwoDThreeD {
  ...
  // the 3D coordinates of the physical board corners, clockwise
   float [][] physicalCorners = {
               {sortedIntersections.get(0).x, sortedIntersections.get(0).y, 0},
               {sortedIntersections.get(1).x, sortedIntersections.get(1).y, 0},
               {sortedIntersections.get(2).x, sortedIntersections.get(2).y, 0},
               {sortedIntersections.get(3).x, sortedIntersections.get(3).y, 0}
              };
  ...
  public TwoDThreeD(int width, int height) {
   ...
   double[][] solveExtrinsicMatrix(List<PVector> points2D) {
    ...
    for(int i=0;i<4;i++){
        points2D.get(i).z = 1;
        projectedCorners[i] = Math.mutiply(invK,points2D.get(i).array());
        // TODO:
